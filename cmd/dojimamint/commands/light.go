...
import (
	...
	lproxy "github.com/dojimanetwork/dojimamint/light/proxy"
	lrpc "github.com/dojimanetwork/dojimamint/light/rpc"
	dbs "github.com/dojimanetwork/dojimamint/light/store/db"
...
func init() {
	...
	LightCmd.Flags().StringVarP(&witnessAddrsJoined, "witnesses", "w", "",
		"dojimamint nodes to cross-check the primary node, comma-separated")
	LightCmd.Flags().StringVar(&home, "home-dir", os.ExpandEnv(filepath.Join("$HOME", ".dojimamint-light")),
	...
	if primaryAddr == "" { // check to see if we can start from an existing state
		...
		if primaryAddr == "" {
			return errors.New("no primary address was provided nor found. Please provide a primary (using -p)." +
				" Run the command: dojimamint light --help for more information")
		}
	} else {
